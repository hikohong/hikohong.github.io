...
p.on('console',m=>{if(m.type()==='error'&&!/ERR_TUNNEL|Failed to load resource/.test(m.text()))errs.push('CON: '+m.text());});
// ── mocks ──
...
checks.push(['SD: candidates=244', /CANDIDATES244|CANDIDATES\s*244/.test(sd.replace(/\s+/g,''))||/244/.test(sd)]);
checks.push(['SD: per-source bars', /RADIO|FRB|TESS/i.test(sd)]);
